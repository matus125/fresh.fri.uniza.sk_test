...
test('vyhladanie_pod_inym_pouzivatelom_ucitel', async ({ page }) => {
...
test('vytvorenie_EN_aktuality_ucitel', async ({ page }) => {
  const title = 'test5'; 
  let success = false;
  attempts = 0;
...
test('vytvorenie_SK/EN_aktuality_ucitel', async ({ page }) => {
  const title = 'test6'; 
  let success = false;
  attempts = 0;
...
test('zmena_text_obrazok_subor_ucitel', async ({ page }) => {
  let success = false;
  attempts = 0;
  while (!success && attempts < 2) { 
    try {
